...
export default function ParentsPage() {
  ...
  const announcements = db
    ...
    .all() as Array<{ id: number; title: string; content: string; category: string; created_at: string }>;
  const homeworks = db
    .prepare("SELECT * FROM homework ORDER BY due_date ASC LIMIT 10")
    .all() as Array<{ id: number; title: string; subject: string; level: string; due_date: string; description: string }>;
  const events = db
